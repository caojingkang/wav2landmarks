...
def process_videos(source_directory, mode, sampling_rate, num_workers, failed_save_file):
    """
    ...
    :return:                      None.
    """
    pool = parallel.Pool(source_directory, mode, sampling_rate, num_workers, failed_save_file)
    pool.start_workers()
    pool.feed_videos()
    pool.stop_workers()
...
def main(args):
    process_videos(args.root, args.mode, args.sampling_rate, args.num_workers, args.failed_log)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'test'], help='traing set or test set')
