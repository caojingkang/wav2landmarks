...
def process_videos(source_directory, mode, num_workers, failed_save_file):
    """
    ...
    :return:                      None.
    """
    pool = parallel.Pool(source_directory, mode, num_workers, failed_save_file)
    pool.start_workers()
    pool.feed_videos()
    pool.stop_workers()
...
def main(args):
    process_videos(args.root, args.mode, args.num_workers, args.failed_log)
