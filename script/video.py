...
def video_to_jpgs(video_path, save_path, do_resize=True, shorter_side=256):
    ...
    if not os.path.isdir(save_path):
        raise OSError('save path {} does not exist', save_path)

    # video = skvideo.io.vread(video_path)
    # for i, frame in enumerate(video):
    #     skimage.io.imsave(
    #         os.path.join(save_path, "frame{:0>4d}.jpg".format(i)),
    #         frame,
    #     )
    ...
    while res:
        ...
        cv2.imwrite(os.path.join(save_path, "frame{:0>4d}.jpg".format(i)), frame, [int(cv2.IMWRITE_JPEG_QUALITY), 75])
        res, frame = cap.read()
        i += 1
